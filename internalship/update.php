...
$id=$_GET['update'];
$s="SELECT `internalship`.id as internalship_id ,`internalship`.title,`internalship`.`image` as internalship_image ,`internalship`.`description`,`internalship`.`address`,`internalship`.start_time,`internalship`.end_time,`instractors`.id as instractor_id,`instractors`.`name` FROM `internalship`JOIN `instractors`ON internalship.insractor_id =`instractors`.id where internalship.id=$id";
...
if(isset($_POST['send'])){
    ...
    if(empty($_FILES['image']['name'])){
        $image_name=$row['internalship_image']; 
    ...
    $u="UPDATE `internalship` SET `title`='$title',`image`='$image_name',`description`='$description',`address`='$address',`start_time`=' $start',`end_time`=' $end',`insractor_id`='$instractor' WHERE internalship.id=$id";
